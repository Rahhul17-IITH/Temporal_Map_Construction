...
def vis_label(dataroot, version, xbound, ybound, thickness, angle_class):
    ...
    for idx in tqdm.tqdm(range(dataset.__len__())):
        ...
        instance_mask = instance_mask.cpu().numpy() if torch.is_tensor(instance_mask) else instance_mask
        instance_mask = instance_mask.sum(0).astype('uint8')
        ...
        R = 1
        arr_width = 1
        forward_mask = forward_mask.cpu().numpy() if torch.is_tensor(forward_mask) else forward_mask
        ...
        backward_mask = backward_mask.cpu().numpy() if torch.is_tensor(backward_mask) else backward_mask
