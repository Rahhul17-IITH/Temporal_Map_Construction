...
def main(args):
    ...
    for rec in tqdm(samples):
        ...
        'map': pred_map.tolist() if isinstance(pred_map, np.ndarray) else pred_map,
        'confidence_level': float(confidence_level) if isinstance(confidence_level, np.number) else confidence_level
    }
        else:
            for vector in vectors:
                # Set a default confidence level if not present
                if 'confidence_level' not in vector:
                    vector['confidence_level'] = 1.0
                else:
                    vector['confidence_level'] = float(vector['confidence_level'])
                # vector['confidence_level'] = float(vector['confidence_level'])
                vector['pts'] = vector['pts'].tolist() if isinstance(vector['pts'], np.ndarray) else vector['pts']
                if 'type' in vector:
                    vector['type'] = int(vector['type'])
