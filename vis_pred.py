    ...
    with torch.no_grad():
        for batchi, (imgs, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, yaw_pitch_roll, segmentation_gt, instance_gt, direction_gt) in enumerate(val_loader):
            ...
            for si in range(segmentation.shape[0]):
                ...
                lidar_vis = lidar_data[si].cpu().numpy()
                plt.imshow(lidar_vis.sum(axis=0), cmap='gray', alpha=0.5)
                
                # Plot vectors with different colors for different types
                coords, types, _ = vectorize(segmentation[si], embedding[si], direction[si], angle_class)
                colors = ['red', 'blue', 'green']  # Different colors for different line types
                for coord, type_idx in zip(coords, types):
                    plt.plot(coord[:, 0], coord[:, 1], color=colors[type_idx], linewidth=5)
