    ...
    with torch.no_grad():
        for batchi, (imgs, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, yaw_pitch_roll, segmentation_gt, instance_gt, direction_gt) in enumerate(tqdm.tqdm(val_loader)):
            ...
            for si in range(segmentation.shape[0]):
                ...
                for coord, confidence, line_type in zip(coords, confidences, line_types):
                    pts = (coord * dx + bx).tolist() if isinstance(coord, np.ndarray) else coord
                    vector = {
                    'pts': pts,
                    'pts_num': int(len(coord)),
                    "type": int(line_type) if isinstance(line_type, np.integer) else line_type,
                    "confidence_level": float(confidence) if isinstance(confidence, (np.floating, np.integer)) else confidence
                    }
                    vectors.append(vector)
